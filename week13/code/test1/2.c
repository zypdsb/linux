...
int main(int argc,char *argv[]){
  ...
  for(i=1;i<NSIG;i++)
  {
    if(i==SIGKILL||i==SIGSTOP||i==32||i==33)continue;
    if(signal(i,&handler)==SIG_ERR)
    {
	fprintf(stderr," signal for signo(%d) error:%s\n",i,strerror(errno));
    }
  }
  if(argc>1)
  {
    ...
    printf("1 will sleep %d second\n",sleep_time);
    sleep(sleep_time);
    sigemptyset(&empty_mask);
    if(sigprocmask(SIG_SETMASK,&empty_mask,NULL)==-1)
    {
	fprintf(stderr," setprocmask to relase all signal failed(%s)\n",strerror(errno));
	return -3;
    }
  }
